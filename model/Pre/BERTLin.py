...
import os
import pandas as pd
from model.Module.LinRec_module_bert import MLPS
...
class BERTLin(SequentialRecommender):
    def __init__(self, conf, training_set, test_set):
        ...
        self.model = LinRec_Model_bert(self.data, self.emb_size, self.max_len, block_num,head_num,drop_rate,self.feature,datasetFile)
        self.rec_loss = torch.nn.BCEWithLogitsLoss()
        self.eps = float(args['-eps'])
        with open("./item-popular/count_office.txt", 'r') as file:
            for line in file:
    ...
    def train(self):
        ...
        # rows_to_adjust = self.find_popular_num()[1][1:]
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(
                ...
                aug_seq, masked, labels = self.item_mask_for_bert(seq, seq_len, self.aug_rate,
                                                                  self.data.item_num + 1)
                ...
                if self.uni == 1:
                    ...
                    uni_item_loss = self.uniformity_loss(y, pos, self.data1)
                elif self.uni == 2:
                ...
                batch_loss = cl_loss + rec_loss + 0.001 * uni_item_loss
                ...
                if n % 50 == 0:
                    ...
                    print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'uni_item_loss:',
                          uni_item_loss, "UNI", UNI.item())
                if n % 200 == 0:
        ...
        # train_los.close()
        # 存一下偏移量
        # with open("./train_HR_and_distance_BERT4Rec_1_office_notadjust5.7.txt", 'a') as train_HR_and_distance:
    ...
    def predict(self, seq, pos, seq_len, gap_batch):
        with torch.no_grad():
            ...
            last_item_embeddings = [seq_emb[i, last - 1, :].view(-1, self.emb_size) for i, last in
                                    enumerate(seq_len)]
    ...
    def draw(self):
        ...
        now = datetime.now()
        plt.savefig('./picture/BERT4Rec/fig' + str(datetime.now()) + '.svg', dpi=300, bbox_inches='tight',
                    format="svg")
        plt.close()
